...
impl FormatBamRecords {
        ...
        match (rec.is_first_in_template(), rec.is_last_in_template()) {
            (true, false) => SerFq {
                ...
                i1: if self.i1_spec.len() > 0 {
                    Some(
                        self.bam_rec_to_fq(rec, &self.i1_spec, self.order[2])
                            .unwrap(),
                    )
                } else {
                    None
                ...
                i2: if self.i2_spec.len() > 0 {
                    Some(
                        self.bam_rec_to_fq(rec, &self.i2_spec, self.order[3])
                            .unwrap(),
                    )
                } else {
                    None
                ...
                &SpecEntry::Tags(ref read_tag, ref qv_tag) => {
                    match rec.aux(read_tag.as_bytes()) {
                        ...
                        None => {
                            ...
                            panic!(format!(
                                "Invalid BAM record: read: {:?} is missing tag: {:?}",
                                std::str::from_utf8(rec.qname()).unwrap(),
                                read_tag
                            ));
                        }
    ...
    > {
        let r1 = self
            .bam_rec_to_fq(r1_rec, &self.r1_spec, self.order[0])
            .unwrap();
        let r2 = self
            .bam_rec_to_fq(r2_rec, &self.r2_spec, self.order[1])
            .unwrap();

        let i1 = if self.i1_spec.len() > 0 {
            Some(
                self.bam_rec_to_fq(r1_rec, &self.i1_spec, self.order[2])
                    .unwrap(),
            )
        } else {
            None
        ...
        let i2 = if self.i2_spec.len() > 0 {
            Some(
                self.bam_rec_to_fq(r1_rec, &self.i2_spec, self.order[3])
                    .unwrap(),
            )
        } else {
            None
...
impl FastqWriter {
    ...
    pub fn try_write_rec(w: &mut Option<BGW>, rec: &Option<FqRecord>) {
        match w {
            &mut Some(ref mut w) => match rec {
                &Some(ref r) => FastqWriter::write_rec(w, r),
                &None => panic!("setup error"),
            },
            &mut None => (),
        }
    }
    ...
    I: Iterator<Item = Result<Record, Error>>,
{
    ...
    let total_read_pairs = {
        ...
        let w: ShardWriter<SerFq, Vec<u8>, SerFqSort> =
